...
public class TableColumnPlus<S, T> extends javafx.scene.control.TableColumn<S, T> {

    private final DoubleProperty percentageWidth = new SimpleDoubleProperty(1);

    public TableColumnPlus() {
        tableViewProperty().addListener(new ChangeListener<TableView<S>>() {
            ...
            public void changed(ObservableValue<? extends TableView<S>> ov, TableView<S> t, TableView<S> t1) {
                if(TableColumnPlus.this.prefWidthProperty().isBound()) {
                    TableColumnPlus.this.prefWidthProperty().unbind();
                }

                TableColumnPlus.this.prefWidthProperty().bind(t1.widthProperty().multiply(percentageWidth));
            }
        });
    }
